...
# NOTE: We intentionally do NOT call st.stop() here when edited_df is empty.
# The unsent email drafts section and email UI must remain accessible even
# when no resumes are loaded in the current session.
if edited_df.empty:
    st.info("No resumes loaded. Upload resumes or add records from the database lookup above.")
else:
    # =========================
    # DB SYNC (only when there's data to sync)
    # =========================
    try:
...
# =========================
# UNSENT EMAIL DRAFTS
# Always rendered — does not depend on resumes being loaded.
# =========================
unsent_db_records = [
