...
# =========================
# USER SIGNATURE (definition only — expander rendered after stats)
# =========================
...
# =========================
# =========================
# STATS GLOBAL FILTERS (date range + recruiter)
# =========================
...
# Determine current user's recruiter identifier (name or email used in DB)
...
with _sf1:
    _stats_date_from = st.date_input(
        "Date From", value=_default_start, key="stats_date_from",
        help="Filters stats cards and the DB table below.",
    )
with _sf2:
    _stats_date_to = st.date_input(
        "Date To", value=_today, key="stats_date_to",
    )
with _sf3:
    _recruiter_options = ["All Recruiters"] + _all_recruiters_in_db
    # Try to pre-select the session user
    ...
    for _i, _opt in enumerate(_recruiter_options):
        if _current_user_recruiter.lower() in _opt.lower() or _opt.lower() in _current_user_recruiter.lower():
            ...
            break
    _stats_recruiter = st.selectbox(
        "Recruiter", options=_recruiter_options,
        index=_default_recruiter_idx, key="stats_recruiter",
        help="Default is the logged-in user. Select 'All Recruiters' to see everyone.",
    )


# ── Helper: parse stored date string "dd-Mon-YYYY" → date ──────────────────
def _parse_record_date(r) -> date:
    try:
...
_email_sent = sum(
    1 for r in _filtered_stats_records if str(r.get("client_email_sent", "No")).strip() in ("Yes", "yes", "1", "true"))
...
_today_email_sent = sum(
    1 for r in _today_records if str(r.get("client_email_sent", "No")).strip() in ("Yes", "yes", "1", "true"))
...
def _mini_stats_row(label: str, icon: str,
                    total, uploaded, pending, emails,
                    row_bg: str, colors: tuple) -> None:
...
# ── Signature expander (moved below stats) ───────────────────────────────────
with st.expander("📝 Manage Your Email Signature"):
    ...
    with _sig_form_col:
        ...
        sig_job_title = st.text_input("Job Title", value=st.session_state.sig_job_title,
                                      placeholder="e.g. Senior recruiter", key="sig_job_title_input")
        sig_phone = st.text_input("Phone Number", value=st.session_state.sig_phone,
                                  placeholder="e.g. +91 0000000000", key="sig_phone_input")
        _user_for_sig = {**user, "name": sig_name or pretty_user_name(user),
                         "job_title": sig_job_title, "phone": sig_phone}
...
filter_source_df = db_df.copy() if not db_df.empty else pd.DataFrame(
    columns=["Candidate Name", "JR Number", "Date", "Actual Status", "Call Iteration", "Upload to SAP", "Email Sent"])

# Apply the stats global filters (date range + recruiter) to the DB table too
if not filter_source_df.empty:
    _dr_from_str = _stats_date_from.strftime("%d-%b-%Y") if _stats_date_from else None
    _dr_to_str = _stats_date_to.strftime("%d-%b-%Y") if _stats_date_to else None


    # Convert Date column to actual date for comparison
    def _safe_parse(v):
        try:
            from datetime import datetime
            return datetime.strptime(str(v), "%d-%b-%Y").date()
        except Exception:
            return None


    _date_parsed = filter_source_df["Date"].apply(_safe_parse)
    _date_mask = pd.Series([True] * len(filter_source_df), index=filter_source_df.index)
    if _stats_date_from:
        _date_mask = _date_mask & _date_parsed.apply(lambda d: d is not None and d >= _stats_date_from)
    if _stats_date_to:
        _date_mask = _date_mask & _date_parsed.apply(lambda d: d is not None and d <= _stats_date_to)
    filter_source_df = filter_source_df[_date_mask]
    if _stats_recruiter != "All Recruiters":
        filter_source_df = filter_source_df[
            filter_source_df["recruiter"].fillna("").astype(str).str.strip().eq(_stats_recruiter) |
            filter_source_df["recruiter_email"].fillna("").astype(str).str.strip().eq(_stats_recruiter)
            ]
...
if not st.session_state.email_drafts_df.empty:
    ...
    if st.button("Send Email", type="primary", width="stretch"):
        ...
        if ok:
            # Mark candidates for this JR as email sent in DB
            _sent_jr = str(draft_row.get("JR Number", "")).strip()
            for _db_rec in st.session_state.db_resume_records:
                if str(_db_rec.get("jr_number", "")).strip() == _sent_jr:
                    _rid = str(_db_rec.get("id", "")).strip()
                    if _rid:
                        try:
                            _upd = dict(_db_rec)
                            _upd["client_email_sent"] = "Yes"
                            update_resume_record(_rid, _upd, user)
                        except Exception:
                            pass
            try:
                st.session_state.db_resume_records = fetch_all_resume_records()
